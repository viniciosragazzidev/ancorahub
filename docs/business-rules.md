...
| BR-029Q | Um lead pode ter no máximo uma oferta ativa por vez e nenhum corretor vira owner antes do aceite. | Oferta criada → a linha do lead é serializada e qualquer oferta ativa concorrente bloqueia nova criação; recusa, expiração, canal indisponível ou falha de enqueue marca a tentativa e avança ao próximo elegível ainda não tentado. Todos tentados sem aceite, ou nenhum caminho automático utilizável → `manual_required`. | DEC-049 |
| BR-029R | A importação CSV preserva a fila automática selecionada e nunca atribui diretamente por uma regra paralela. | Importação válida → servidor revalida tenant, unidade, estado e modo da fila. Se a fila desativou Qualificação IA, o lead entra como `qualified`/`COMPLETED`, sem owner e com job idempotente imediato; o motor oferece pelo WhatsApp oficial a um elegível por vez até aceite ou esgotamento. | DEC-096 |
