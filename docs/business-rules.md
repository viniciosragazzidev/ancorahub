...
| BR-023 | Novo lead elegível entra em um ciclo sequencial de ofertas entre corretores elegíveis da fila/unidade. | Lead recebido → um único corretor recebe a oferta e vira owner provisório para enxergar o lead na carteira. Aceite confirma a titularidade: nenhum fluxo automático (worker, SLA de primeiro contato ou exclusão do owner) retira o lead do corretor que aceitou; o SLA apenas alerta corretor e gestão. Recusa/expiração libera ou transfere atomicamente ao próximo ainda não tentado. | RF020–021, RF102, DEC-049, DEC-104 |
