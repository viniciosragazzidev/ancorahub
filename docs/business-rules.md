...
| BR-023 | Novo lead elegível entra em um ciclo sequencial de ofertas entre corretores elegíveis da fila/unidade. | Lead recebido → um único corretor recebe a oferta, mas o lead permanece sem owner até o aceite atômico. Recusa/expiração avança atomicamente ao próximo ainda não tentado; somente o aceite cria a titularidade definitiva. | RF020–021, RF102, DEC-049, DEC-096 |
...
| BR-029M | Distribuição automática garante processamento 24/7; a janela comercial regula somente a entrega de mensagens. | Lead elegível entra → o motor central resolve unidade/fila e cria uma oferta exclusiva; o lead permanece sem owner até o aceite. Sem elegível ou em pausa explícita → mantém o estado de fila e a task reavalia. | DEC-083 (emendada por DEC-096) |
| BR-029S | Nenhum lead operacional pode permanecer sem corretor enquanto houver corretor elegível em sua unidade. | Entrada sem unidade → unidade automática de menor carga; unidade operacional definida com auto desligado → reativação auditada; fila inativa → reparo para política global; seleção → disponibilidade, canal, plantão, política, menor fila sem contato e menor carga; capacidade cheia → continua pelo ranking. Recusa, expiração ou SLA → troca atômica para o próximo elegível, sem estado intermediário sem owner. | DEC-097 |
| BR-029N | Avisos oficiais consecutivos ao mesmo corretor respeitam cadência. | Cada `new_lead_broker` aguarda pelo menos 10 minutos após o último aviso ao mesmo corretor; eventos preservam ordem e não são descartados. A outbox registra o reagendamento e os webhooks persistem apenas o código e título seguro de falhas de entrega da Meta. | DEC-084 |
...
| BR-029Q | Um lead pode ter no máximo uma oferta ativa por vez e nenhum corretor vira owner antes do aceite. | Oferta criada → a linha do lead é serializada e somente a oferta ligada a uma mensagem durável na outbox é considerada ativa. Tentativa sem mensagem, recusa, expiração, canal indisponível ou falha de enqueue não bloqueia o lead e avança ao próximo elegível ainda não tentado. Todos tentados sem aceite → inicia outro ciclo automático preservando o histórico; sem elegível → permanece em retry auditável. | DEC-049 |
| BR-029R | A importação CSV preserva a fila automática selecionada e nunca atribui diretamente por uma regra paralela. | Importação válida → servidor revalida tenant, unidade, estado e modo da fila, ativa a auto-distribuição da unidade operacional e reativa a política exata da fila quando necessário. Se a fila desativou Qualificação IA, o lead entra como `qualified`/`COMPLETED`, sem owner e com job idempotente imediato; o motor oferece pelo WhatsApp oficial a um elegível por vez até aceite. Unidade pausada ou Central de redistribuição bloqueia a importação antes de criar leads. | DEC-096 |
