...
| BR-029O | Avisos internos a corretores podem usar o número WAHA oficial do tenant. | Diretor escolhe número WAHA ativo de escopo tenant e o modo `meta_then_waha` ou `waha_direct`; somente eventos internos de atribuição/oferta são elegíveis. Falha confirmada da Meta pode disparar uma única contingência WAHA, sem afetar atendimento de leads, qualificação ou campanhas. Configuração e envio são auditáveis e o Super-admin possui kill switch global. | DEC-089 |
