...
| BR-029M | Distribuição automática garante processamento 24/7; a janela comercial regula somente a entrega de mensagens. | Lead elegível entra → o motor central resolve unidade/fila, cria uma oferta exclusiva e vincula o owner provisório. Sem elegível ou em pausa explícita → mantém o estado de fila e a task reavalia. | DEC-083, DEC-104 |
| BR-029S | Nenhum lead operacional pode permanecer sem corretor enquanto houver corretor elegível em sua unidade. | Entrada sem unidade → unidade automática de menor carga; unidade operacional definida com auto desligado → reativação auditada; fila inativa → reparo para política global; seleção → disponibilidade, canal, plantão, política, menor fila sem contato e menor carga; capacidade cheia → continua pelo ranking. A oferta cria owner provisório visível; recusa, expiração ou SLA → troca atômica para o próximo elegível, sem estado intermediário órfão. | DEC-097, DEC-104 |
