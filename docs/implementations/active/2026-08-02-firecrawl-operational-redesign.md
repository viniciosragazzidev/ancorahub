...
Esta é a primeira onda do plano em `docs/product/ancorahub-redesign-master-plan.md`. Ela afeta todas as rotas por herança dos primitives e shells, mas não substitui a revisão de composição específica de cada domínio. As próximas ondas devem migrar, com screenshots e estados completos, Leads/Conversas/Tarefas, gestão, configurações, Super Admin, autenticação e extensão.
