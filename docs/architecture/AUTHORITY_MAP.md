...
| **Autorização, RBAC & Escopo** | FRAGMENTED (Modelado em `authorization-service.ts`, executado via `context.role ===` ad-hoc) | HIGH | `custom-roles/service.ts`, `equipe/actions.ts` | Server Actions (>250 arquivos), layout guards | `/equipe/cargos` | **CRITICAL** |
