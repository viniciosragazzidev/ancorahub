...
async function cachedResolve(resolve, sessionName, lid, now) {
...
export async function attachLidPhoneNumbers(body, resolve, now = Date.now()) {
    const result = { from: false, to: false };
    if (!body || typeof body !== "object")
        return result;
    const envelope = body;
    if (!MESSAGE_EVENTS.has(String(envelope.event)) || typeof envelope.session !== "string")
        return result;
    ...
    if (Object.keys(attached).length) {
        const existing = payload._ancora && typeof payload._ancora === "object" ? payload._ancora : {};
