...
type ForcedSessionReconnect = Awaited<ReturnType<WahaClient["reconnectSession"]>>;
const forcedSessionReconnects = new Map<string, Promise<ForcedSessionReconnect>>();
...
function reconnectSessionOnce(client: WahaClient, sessionName: string) {
  const existing = forcedSessionReconnects.get(sessionName);
  if (existing) return existing;

  const reconnect = client.reconnectSession(sessionName).finally(() => {
    if (forcedSessionReconnects.get(sessionName) === reconnect) forcedSessionReconnects.delete(sessionName);
  });
...
export function buildApp() {
  ...
  app.post<{ Body: SendBody }>("/api/integrations/whatsapp/send-test-message", {
    ...
    if (!requireInternalAuth(request, reply, config.internalToken)) return;
  ...
  app.get("/internal/waha/test-session/qr", async (request, reply) => {
    ...
    const client = new WahaClient(wahaConfig);
    const sessionName = "corretop_test";      try {
        const session = await client.getSession(sessionName);
        const status = session?.status ?? "DISCONNECTED";
  ...
  app.post<{ Body: { tenantId: string; userId: string; sessionName: string } }>(
  ...
  app.post<{ Params: { id: string } }>(
    "/internal/waha/connections/:id/reconnect",
    {
      schema: {
        params: {
          type: "object",
  ...
  app.get<{ Params: { id: string } }>(
    "/internal/waha/connections/:id/status",
    {
      schema: {
        params: {
          type: "object",
  ...
  app.get<{ Params: { id: string } }>(
    "/internal/waha/connections/:id/state",
    {
      schema: {
        params: {
          type: "object",
  ...
  app.get<{ Params: { id: string }; Querystring: { limit?: string } }>(
    "/internal/waha/connections/:id/chats",
    {
      schema: {
        params: {
          type: "object",
  ...
  app.get<{ Params: { id: string } }>(
    "/internal/waha/connections/:id/qr",
    {
      schema: {
        params: {
          type: "object",
    ...
    async (request, reply) => {
      ...
      try {
        ...
        request.log.info({
          ...
          accepted: (result as Record<string, unknown>).accepted,
        });
