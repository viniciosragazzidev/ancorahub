...
import { completeBrokerAvailabilityOnboarding, saveOwnBrokerAvailability, skipBrokerAvailabilityOnboarding } from "./service";
...
export async function skipBrokerAvailabilityOnboardingAction() {
  const result = await skipBrokerAvailabilityOnboarding();
  revalidatePath("/dashboard", "layout");
  return result;
}
