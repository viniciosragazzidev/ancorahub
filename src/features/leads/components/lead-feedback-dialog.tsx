    ...
    <DialogPopup key={open ? "open" : "closed"} className="overflow-hidden p-0 sm:max-w-xl">
      <DialogPanel className="gap-0">
        ...
        <form action={submit} className="grid gap-5 px-5 py-5 sm:px-6">
          {step === 1 && <section className="t-feedback-step grid gap-3"><p className="text-sm font-medium">O que aconteceu?</p><div className="grid grid-cols-2 gap-2">{events.map(([value, label]) => <Button key={value} type="button" onClick={() => setEventType(value)} className={`h-auto justify-start px-3 py-3 text-left text-sm ${eventType === value ? "border-primary bg-primary/10 text-primary" : ""}`} variant="outline"><span className="flex w-full items-center justify-between gap-2">{label}{eventType === value && <Check size={16} weight="bold" />}</span></Button>)}</div></section>}
          {step === 2 && <section className="t-feedback-step grid gap-3"><p className="text-sm font-medium">Como o cliente reagiu?</p><div className="grid gap-2">{reactions.map(([value, label]) => <Button key={value} type="button" onClick={() => setReaction(value)} className={`h-auto justify-start px-3 py-3 text-left text-sm ${reaction === value ? "border-primary bg-primary/10 text-primary" : ""}`} variant="outline"><span className="flex w-full items-center justify-between gap-2">{label}{reaction === value && <Check size={16} weight="bold" />}</span></Button>)}</div></section>}
