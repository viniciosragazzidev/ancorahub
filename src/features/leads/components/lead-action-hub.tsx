...
function getFallbackAction({
  ...
  status,
  hasPendingDocuments,
  canAccessConversas = false,
}: Pick<
  LeadActionHubProps,
  "leadId" | "status" | "hasPendingDocuments" | "canAccessConversas"
>): Action {
  if (hasPendingDocuments)
    return { href: "#documentos", label: "Revisar documentos", icon: FileText };
  if (status === "distributed" || status === "new" || !canAccessConversas)
    return { href: "#lead-actions", label: "Ver ação do atendimento", icon: ChatCircleText };
  if (status === "converted")
    return { href: "/clientes", label: "Acompanhar cliente", icon: CheckCircle };
  return {
    href: "/conversas?leadId=" + leadId + "&draft=broker_intro",
...
export function LeadActionHub({
  ...
  const isOverdue = Boolean(
    nextTask?.dueAt && new Date(nextTask.dueAt).getTime() < getCurrentTimestamp(),
  );

  return (
    <section
      aria-labelledby="lead-next-action-title"
      className="rounded-xl border border-primary/20 bg-card p-4 shadow-sm sm:p-5"
    >
      {/* Progress Stepper */}
      <div className="mb-4">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <h2
            ...
            className="mt-1 max-w-2xl text-base font-semibold tracking-tight"
          >
            {nextTask ? nextTask.title : fallbackAction.label}
          </h2>
          <p className="mt-1 max-w-xl text-sm leading-5 text-muted-foreground">
            {nextTask
              ? "O próximo passo pendente para manter o atendimento em movimento."
              : "Recomendação baseada no status e nas pendências deste lead."}
          </p>
        </div>
        <Button
      ...
      <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-primary/10 pt-3 text-xs text-muted-foreground">
        <Badge variant={isOverdue ? "destructive" : "outline"}>
          ...
          {nextTask ? formatDueAt(nextTask.dueAt) : "Sem tarefa pendente"}
        </Badge>
        <span>
          Responsável:{" "}
          <strong className="font-medium text-foreground">
            {nextTask?.assigneeName ?? currentOwner ?? "A definir"}
          </strong>
        </span>
        {nextTask ? (
          <span>
      ...
      <nav aria-label="Atalhos do lead" className="mt-3 flex flex-wrap gap-2">
        ...
        {canAccessConversas ? (
          <Button
            className="h-8 px-2.5 text-xs"
            render={<Link href={"/conversas?leadId=" + leadId + "&draft=broker_intro"} />}
            size="sm"
            variant="outline"
          >
            <ChatCircleText className="size-4" /> Conversar
          </Button>
        ) : null}
        <LeadQuickNote leadId={leadId} />
        <Button
          className="h-8 px-2.5 text-xs"
          render={<Link href={"/tarefas?leadId=" + leadId} />}
          size="sm"
          variant="outline"
        >
        ...
        <Button
          className="h-8 px-2.5 text-xs"
          render={<Link href="#documentos" />}
          size="sm"
          variant="outline"
        >
          <FileText className="size-4" /> Documentos{hasPendingDocuments ? " · pendentes" : ""}
        </Button>
      </nav>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  O SLA foi atualizado. Registre outro se necessário.
                </p>
              </div>
            </div>
            <Button
