...
export function MetaAssetsReview({ assets, error }: { assets: MetaDiscoveredAssets; error?: string | null }) {
  return <div className="space-y-3 py-2"><Card><CardHeader className="p-3 pb-1"><CardTitle className="flex items-center gap-2 text-sm"><Globe className="size-4 text-primary" />Business Manager</CardTitle></CardHeader><CardContent className="p-3 pt-1"><p className="font-medium">{assets.business.name}</p><p className="font-mono text-xs text-muted-foreground">{assets.business.id}</p></CardContent></Card><AssetCard icon={<UserList className="size-4 text-chart-3" />} title={`Páginas (${assets.pages.length})`} items={assets.pages.map((asset) => ({ id: asset.id, label: asset.name }))} empty="Nenhuma página autorizada." /><AssetCard icon={<SquaresFour className="size-4 text-chart-2" />} title={`Contas de anúncios (${assets.adAccounts.length})`} items={assets.adAccounts.map((asset) => ({ id: asset.id, label: asset.name, meta: asset.currency }))} empty="Nenhuma conta de anúncios autorizada." />{error ? <p role="alert" className="text-sm text-destructive">{error}</p> : null}</div>;
}
...
export function MetaAssetsModal({ open, onOpenChange, assets, onConfirm }: {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogPopup className="max-h-[85vh] max-w-xl overflow-y-auto"><DialogHeader><Badge variant="outline" className="w-fit bg-emerald-500/10 text-emerald-700">OAuth de Marketing</Badge><DialogTitle>Ativos autorizados</DialogTitle><DialogDescription>Confirme somente os ativos da corretora. Esta etapa grava a conexão e ativa a sincronização; o WhatsApp possui um conector independente.</DialogDescription></DialogHeader><MetaAssetsReview assets={assets} error={error} /><DialogFooter><Button variant="outline" size="sm" onClick={() => onOpenChange(false)} disabled={submitting}>Cancelar</Button><Button size="sm" onClick={() => void confirm()} disabled={submitting}><CheckCircle className="size-4" />{submitting ? "Conectando..." : "Confirmar e conectar"}</Button></DialogFooter></DialogPopup></Dialog>;
}
