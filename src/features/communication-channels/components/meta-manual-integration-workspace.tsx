...
function TutorialDialog({ webhookUrl, verifyTokenConfigured, onComplete }: { webhookUrl: string; verifyTokenConfigured: boolean; onComplete: () => void }) {
...
export function MetaManualIntegrationWorkspace(props: Props) {
  ...
  const diagnostics = [
    ...
    ["Página e marketing", props.settings?.facebookPageId && props.settings?.adAccountId ? "ok" : "warning", props.settings?.facebookPageId && props.settings?.adAccountId ? "IDs cadastrados" : "Opcional nesta etapa"],
  ] as const;

  return <TooltipProvider><main className="flex min-h-full flex-col gap-6 bg-background p-4 lg:p-6">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-xs font-medium text-primary">CONFIGURAÇÕES • INTEGRAÇÕES</p><h1 className="mt-1 text-2xl font-semibold tracking-tight">Meta Business</h1><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Conecte a conta da empresa, valide o WhatsApp oficial e mantenha os dados de marketing organizados no mesmo lugar.</p></div><TutorialDialog webhookUrl={props.webhookUrl} verifyTokenConfigured={props.verifyTokenConfigured} onComplete={() => void completeManualMetaTutorialAction().then(() => toast.success("Tutorial marcado como concluído."))} /></div>
    ...
    <Tabs defaultValue="general">
      ...
      <TabsContent value="marketing" className="pt-6"><Card className="border-border shadow-none"><CardHeader><CardTitle>Marketing e captação</CardTitle><CardDescription>IDs cadastrados para preparar integrações futuras de campanhas, formulários e conversões.</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2"><KeyValue label="Página" value={props.settings?.facebookPageId ?? "Não configurada"} /><KeyValue label="Conta de anúncios" value={props.settings?.adAccountId ?? "Não configurada"} /><KeyValue label="Pixel" value={props.settings?.pixelId ?? "Não configurado"} /><KeyValue label="Dataset" value={props.settings?.datasetId ?? "Não configurado"} /></CardContent></Card></TabsContent>
