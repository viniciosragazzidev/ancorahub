...
function TutorialDialog({ webhookUrl, verifyTokenConfigured, onComplete, leadAdsWebhookUrl }: { webhookUrl: string; verifyTokenConfigured: boolean; onComplete: () => void; leadAdsWebhookUrl: string }) {
  ...
  return <Dialog>
    ...
    <DialogPopup className="max-w-3xl p-0">
      <DialogHeader className="border-b border-border p-6"><DialogTitle>Assistente de configuração Meta</DialogTitle><p className="text-sm text-muted-foreground">Escolha a trilha que precisa configurar. WhatsApp e anúncios usam dados e webhooks diferentes.</p></DialogHeader>
      <ScrollArea className="max-h-[65dvh] px-6 py-5"><div className="space-y-5">
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground"><strong className="text-foreground">Antes de começar:</strong> o aplicativo AncoraHub já existe. Não crie outro app, não envie App Secret por mensagem e não coloque o token técnico de anúncios dentro do CRM.</div>
        <section className="space-y-4 rounded-xl border border-border p-4"><div><Badge variant="info">Trilha 1</Badge><p className="mt-2 font-semibold text-foreground">WhatsApp oficial da empresa</p><p className="mt-1 text-sm leading-6 text-muted-foreground">A empresa informa os dados do próprio número para o Diretor conectar neste CRM.</p></div>
          {whatsappSteps.map(([title, body], index) => <div key={title} className="flex gap-4"><Badge variant="outline" className="mt-0.5 size-6 justify-center rounded-full p-0">{index + 1}</Badge><div><p className="font-medium text-foreground">{title}</p><p className="mt-1 text-sm leading-6 text-muted-foreground">{body}</p>{index === 0 ? <Button render={<a href="https://developers.facebook.com/" rel="noreferrer" target="_blank" />} className="mt-2" size="sm" variant="outline">Abrir Meta Developers</Button> : null}{index === 3 ? <div className="mt-3 space-y-2"><CopyValue value={webhookUrl} label="Webhook do WhatsApp" /><CopyValue value={verifyTokenConfigured ? "Solicite o Verify Token ao administrador da plataforma" : "Aguardando configuração do ambiente AncoraHub"} label="Verify Token" /></div> : null}</div></div>)}</section>
        <section className="space-y-4 rounded-xl border border-border p-4"><div><Badge variant="info">Trilha 2</Badge><p className="mt-2 font-semibold text-foreground">Formulários de anúncios (Lead Ads)</p><p className="mt-1 text-sm leading-6 text-muted-foreground">O cliente libera a Página; o administrador da plataforma mantém um único token técnico para todos os clientes.</p></div>
          <ol className="space-y-3 text-sm leading-6 text-muted-foreground"><li><strong className="text-foreground">1. Cliente:</strong> em Configurações do Negócio, adiciona o parceiro AncoraHub e compartilha a Página e, se aplicável, a conta de anúncios.</li><li><strong className="text-foreground">2. Cliente/Administrador:</strong> na Página, abre Integrações → Acesso a Leads e libera o aplicativo AncoraHub em CRMs.</li><li><strong className="text-foreground">3. Administrador AncoraHub:</strong> libera esses ativos para o Usuário do Sistema e mantém o token técnico apenas na Vercel. Este token é cadastrado uma vez, não por cliente.</li><li><strong className="text-foreground">4. Diretor:</strong> abre a aba Marketing, informa o Page ID e escolhe a unidade de destino. Sem unidade, o lead entra na fila geral.</li><li><strong className="text-foreground">5. Administrador AncoraHub:</strong> no Meta Developers, assina <code>leadgen</code> no objeto <code>Page</code> usando a URL abaixo.</li></ol>
          <CopyValue value={leadAdsWebhookUrl} label="Webhook de Lead Ads" />
        </section>
        <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-border p-4 text-sm"><input checked={checked} onChange={(event) => setChecked(event.target.checked)} type="checkbox" className="mt-0.5 size-4 accent-primary" /><span><strong className="text-foreground">Entendi as responsabilidades</strong><br />WhatsApp usa os dados da empresa; Lead Ads usa uma Página compartilhada e o token técnico da plataforma nunca é enviado ao cliente.</span></label>
      </div></ScrollArea>
