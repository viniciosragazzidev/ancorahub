...
function formatBytes(bytes: number | undefined) {
  ...
  return `${val >= 10 || exponent === 0 ? val.toFixed(0) : val.toFixed(1)} ${units[exponent]}`;
}
...
function RowAction({
  ...
  if (state === "complete") {
    return (
      <Tooltip content="Upload concluído" side="top" delay={100}>
        <motion.span
          role="status"
  ...
  if (state === "removing") {
    return (
      <Tooltip content="Removendo anexo" side="top" delay={100}>
        <span
          role="status"
          aria-label={`Removendo ${label}`}
  ...
  if (state === "failed") {
    if (!retryable) {
      return (
        <Tooltip content="Falha no upload" side="top" delay={100}>
          <span
            role="status"
            aria-label={`Falha no upload para ${label}`}
    ...
    return (
      <Tooltip content="Tentar novamente" side="top" delay={100}>
        <motion.button
          type="button"
  ...
  return (
    <Tooltip content="Remover anexo" side="top" delay={100}>
      <motion.button
        type="button"
...
export function AttachmentUpload({
  ...
  disabled = false,
  title = "Arraste e solte ou escolha os arquivos",
  description,
  ...
  return (
    <LayoutGroup id={inputId}>
      ...
      <motion.button
        ...
        className={cn(
          "group relative isolate flex min-h-36 w-full flex-col items-center justify-center overflow-hidden rounded-2xl bg-muted/50 p-4 text-center outline-none",
      ...
      >
        <span
          aria-hidden="true"
          className="absolute inset-1.5 -z-10 rounded-xl border border-dashed border-muted-foreground/25 bg-background/50 transition-[border-color,background-color] duration-200 group-hover:border-muted-foreground/45 group-data-[dragging=true]:border-foreground/65 group-data-[dragging=true]:bg-muted/20"
        />
        <motion.span
          aria-hidden="true"
          animate={
        ...
        <span className="text-xs font-semibold tracking-[-0.01em] text-foreground">
          {maxReached ? "Limite de anexos atingido" : title}
        </span>
        <span className="mt-0.5 text-[11px] leading-4 text-muted-foreground">
          {maxReached
            ? `${items.length} de ${maxFiles} anexos adicionados`
            : description ?? `Tamanho máximo de ${formatMaxSize(maxFileSize)}`}
        </span>
      </motion.button>

      {items.length > 0 ? (
        <section className="mt-4" aria-labelledby={`${inputId}-attachments`}>
          <h3
          ...
          <ul className={cn("mt-2 space-y-2", classNames?.list)}>
