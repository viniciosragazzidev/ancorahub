...
function formatBytes(bytes: number) {
  ...
  return `${val >= 10 || exponent === 0 ? val.toFixed(0) : val.toFixed(1)} ${
    units[exponent]
  }`;
}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div
          className={cn(
            "grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-muted text-muted-foreground",
            classNames?.leading,
          )}
        >
        ...
        <div className={cn("min-w-0 flex-1", classNames?.content)}>
          ...
          {showProgress ? (
            <div
              ...
              aria-label={`Progresso de envio de ${item.name}`}
              className={cn(
...
export function FileUpload({
  ...
  description = "Adicione arquivos à fila de envio",
    ...
    <div className={cn("w-full space-y-3", className, classNames?.root)}>
      ...
      <button
        ...
        className={cn(
          ...
          centered
            ? "min-h-44 flex-col items-center justify-center gap-2 p-6 text-center"
            : "items-center gap-3 p-4 text-left",
          classNames?.dropzone,
        )}
      >
        <motion.span
          ...
          className={cn(
            ...
            centered
              ? "h-12 w-12 rounded-xl border border-border"
        ...
        <span className={cn("min-w-0", centered ? "max-w-xs" : "flex-1")}>
          <span
            className={cn(
              ...
              centered ? "text-sm" : "text-xs",
            )}
          >
            {maxReached ? "Limite de envio atingido" : title}
          </span>
          <span
            className={cn(
              "block text-[11px] text-muted-foreground",
              centered ? "mt-0.5 leading-4" : "mt-0.5",
            )}
          >
            {maxReached
              ? `${items.length} de ${maxFiles} arquivos adicionados`
              : description}
          </span>
        </span>
        ...
        <span
          className={cn(
            ...
            centered ? "mt-1 px-3.5 py-1.5" : "px-3 py-1.5",
          )}
        >
          {browseLabel}
        </span>
