...
function SummaryCard({ icon: Icon, label, value, description, tone = "neutral" }: {
  ...
  return <Card className={`gap-0 p-0 ${toneClass}`}><CardContent className="flex min-h-20 items-center gap-3 p-3"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-muted text-muted-foreground"><Icon className="size-4" /></span><div className="min-w-0"><p className="text-xs font-medium text-muted-foreground">{label}</p><div className="mt-0.5 flex items-baseline gap-2"><p className="text-lg font-semibold tabular-nums text-foreground">{value}</p><p className="truncate text-xs text-muted-foreground">{description}</p></div></div></CardContent></Card>;
}
...
function DutyTimeline({ schedules, assignments, onOpen }: { schedules: Schedule[]; assignments: Assignment[]; onOpen: (schedule: Schedule) => void }) {
  return <div className="overflow-x-auto pb-1"><div className="grid min-w-[1540px] grid-cols-7 overflow-hidden rounded-xl border border-border/70 bg-border/70">{DAYS.map((day, dayIndex) => {
...
function DutyFormSheet({ open, onOpenChange, schedule, snapshot }: { open: boolean; onOpenChange: (open: boolean) => void; schedule: Schedule | null; snapshot: Snapshot }) {
  ...
  const [branchId, setBranchId] = useState(schedule?.branchId ?? snapshot.branches[0]?.id ?? "");
  const [queueId, setQueueId] = useState(schedule?.queueId ?? "");
  const branchQueues = snapshot.queues.filter((queue) => queue.branchId === branchId);
  const activeQueueId = branchQueues.some((queue) => queue.id === queueId) ? queueId : branchQueues[0]?.id ?? "";
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    formData.set("queueId", activeQueueId);
    if (schedule) formData.set("scheduleId", schedule.id);
    const action: DutyAction = schedule ? updateDutyScheduleAction : createDutyScheduleAction;
    startTransition(async () => { const result = await action({}, formData); if (!result.success) { toast.error(result.error ?? "Não foi possível salvar o plantão."); return; } toast.success(schedule ? "Plantão atualizado." : "Plantão criado."); onOpenChange(false); });
  }

  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent><SheetHeader><SheetTitle>{title}</SheetTitle><SheetDescription>Defina a regra recorrente, a fila e o mínimo de corretores para cobertura.</SheetDescription></SheetHeader><SheetBody><form className="grid gap-5" onSubmit={submit}><div className="grid gap-2"><Label htmlFor="duty-name">Nome</Label><Input id="duty-name" name="name" defaultValue={schedule?.name ?? ""} placeholder="Ex.: Plantão comercial" required /></div><div className="grid gap-2"><Label htmlFor="duty-branch">Unidade</Label><select id="duty-branch" value={branchId} onChange={(event) => { const nextBranch = event.target.value; setBranchId(nextBranch); setQueueId(snapshot.queues.find((queue) => queue.branchId === nextBranch)?.id ?? ""); }} className="h-9 rounded-lg border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring">{snapshot.branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></div><div className="grid gap-2"><Label htmlFor="duty-queue">Fila</Label><select id="duty-queue" value={activeQueueId} onChange={(event) => setQueueId(event.target.value)} disabled={!branchQueues.length} className="h-9 rounded-lg border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring">{branchQueues.map((queue) => <option key={queue.id} value={queue.id}>{queue.name}</option>)}</select>{!branchQueues.length && <p className="text-xs text-warning">Esta unidade não possui uma fila ativa.</p>}</div><div className="grid grid-cols-2 gap-3"><div className="grid gap-2"><Label htmlFor="duty-day">Dia</Label><select id="duty-day" name="dayOfWeek" defaultValue={String(schedule?.dayOfWeek ?? 1)} className="h-9 rounded-lg border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring">{DAYS_FULL.map((day, index) => <option key={day} value={index}>{day}</option>)}</select></div><div className="grid gap-2"><Label htmlFor="duty-priority">Prioridade</Label><Input id="duty-priority" name="priority" type="number" min={1} max={999} defaultValue={schedule?.priority ?? 100} required /></div></div><div className="grid grid-cols-2 gap-3"><div className="grid gap-2"><Label htmlFor="duty-start">Início</Label><Input id="duty-start" name="startsAt" type="time" defaultValue={schedule?.startsAt.slice(0, 5) ?? "09:00"} required /></div><div className="grid gap-2"><Label htmlFor="duty-end">Fim</Label><Input id="duty-end" name="endsAt" type="time" defaultValue={schedule?.endsAt.slice(0, 5) ?? "18:00"} required /></div></div><div className="grid gap-2"><Label htmlFor="duty-minimum">Mínimo de corretores</Label><Input id="duty-minimum" name="minimumBrokers" type="number" min={1} max={99} defaultValue={schedule?.minimumBrokers ?? 1} required /><p className="text-xs text-muted-foreground">Abaixo deste mínimo, o plantão vira uma pendência; a distribuição não é bloqueada.</p></div><div className="grid grid-cols-2 gap-3"><div className="grid gap-2"><Label htmlFor="duty-valid-from">Início da vigência</Label><Input id="duty-valid-from" name="validFrom" type="date" defaultValue={dateInputValue(schedule?.validFrom ?? new Date())} required /></div><div className="grid gap-2"><Label htmlFor="duty-valid-until">Fim da vigência</Label><Input id="duty-valid-until" name="validUntil" type="date" defaultValue={dateInputValue(schedule?.validUntil ?? null)} /></div></div><div className="grid gap-2"><Label htmlFor="duty-credential">Origem de lead</Label><select id="duty-credential" name="webhookCredentialId" defaultValue={schedule?.webhookCredentialId ?? ""} className="h-9 rounded-lg border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"><option value="">Todas as origens</option>{snapshot.credentials.map((credential) => <option key={credential.id} value={credential.id}>{credential.name}</option>)}</select></div><p className="rounded-lg border border-muted bg-muted/30 px-3 py-2 text-xs text-muted-foreground">Fuso operacional: America/Sao_Paulo. Regras com mesma prioridade, fila, dia e horário não podem coexistir ativas.</p><Button type="submit" disabled={pending || !activeQueueId}>{pending ? "Salvando…" : schedule ? "Salvar alterações" : "Criar plantão"}</Button></form></SheetBody></SheetContent></Sheet>;
}
