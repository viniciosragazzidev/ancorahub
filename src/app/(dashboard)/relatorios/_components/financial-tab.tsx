...
function SummaryTable({
  ...
  rows: readonly { label: string; revenue: number; sales: number }[];
}) {
  ...
  return (
    <section aria-labelledby={`fin-${title.toLowerCase().replace(/\s+/g, "-")}-title`}>
      <div className="mb-3 flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <h2
          id={`fin-${title.toLowerCase().replace(/\s+/g, "-")}-title`}
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.label}>
                    ...
                    <TableCell className="text-right tabular-nums">
                      {row.sales > 0
                        ? (row.revenue / row.sales).toLocaleString("pt-BR", {
                            style: "currency",
                            currency: "BRL",
                          })
                        : "—"}
          ...
          <MobileDataList>
            {rows.map((row) => (
              <MobileDataListItem key={row.label}>
                <h3 className="mb-2.5 truncate text-sm font-semibold text-foreground">
                  {row.label}
                </h3>
                <div className="space-y-2">
                  ...
                  <MobileDataRow
                    label="Receita"
                    value={row.revenue.toLocaleString("pt-BR", {
                      style: "currency",
                      currency: "BRL",
                    })}
                  />
    ...
    <>
      <section aria-labelledby="financial-overview-title">
        <div className="mb-3 flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <h2 id="financial-overview-title" className="font-medium text-foreground">
            Resumo financeiro
          </h2>
          <span aria-hidden="true">•</span>
          <span>Últimos {period} dias</span>
        </div>
        <DashboardGrid className="sm:grid-cols-3">
