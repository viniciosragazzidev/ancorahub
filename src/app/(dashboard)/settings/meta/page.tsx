...
import { TenantDebugBar } from "@/features/communication-channels/components/tenant-debug-bar";
...
export default async function MetaIntegrationSettingsPage() {
  ...
  const [channel, sources, identity, pilotEnabled, tenantInfo] = await Promise.all([
    ...
    db.select({ name: schema.tenants.name }).from(schema.tenants).where(eq(schema.tenants.id, context.tenantId)).limit(1).then(r => r[0] ?? null),
  ]);
