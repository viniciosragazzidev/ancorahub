    ...
    <main className="mx-auto flex min-h-full w-full max-w-[1440px] flex-col gap-5 bg-background p-4 lg:p-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,.85fr)]">
        ...
        <div className="space-y-5">
          <Card className="border-transparent bg-transparent shadow-none"><CardHeader className="pb-3"><CardTitle className="text-base">Resultado comercial</CardTitle><CardDescription>Produção e resposta às ofertas registradas no CRM.</CardDescription></CardHeader><CardContent className="grid grid-cols-2 gap-4 text-sm"><div><p className="text-xs text-muted-foreground">Ofertas enviadas</p><p className="mt-1 font-mono text-xl font-semibold">{metrics.offers.total}</p><p className="text-xs text-muted-foreground">{metrics.offers.accepted} aceita(s) · {metrics.offers.pending} pendente(s)</p></div><div><p className="text-xs text-muted-foreground">Não aceitas</p><p className="mt-1 font-mono text-xl font-semibold">{metrics.offers.notAccepted}</p><p className="text-xs text-muted-foreground">{metrics.offers.declined} recusada(s) · {metrics.offers.expired} expirada(s)</p></div><div><p className="text-xs text-muted-foreground">Cotações criadas</p><p className="mt-1 font-mono text-xl font-semibold">{metrics.quotes.total}</p><p className="text-xs text-muted-foreground">{metrics.quotes.sent} enviadas</p></div><div><p className="text-xs text-muted-foreground">Vendas</p><p className="mt-1 font-mono text-xl font-semibold">{metrics.sales.total}</p><p className="text-xs text-muted-foreground">{metrics.sales.cancelled} cancelada(s)</p></div><div><p className="text-xs text-muted-foreground">Atividades</p><p className="mt-1 font-mono text-xl font-semibold">{metrics.interactions}</p><p className="text-xs text-muted-foreground">Interações registradas</p></div><div><p className="text-xs text-muted-foreground">Tarefas concluídas</p><p className="mt-1 font-mono text-xl font-semibold">{metrics.tasks.completed}</p><p className="text-xs text-muted-foreground">de {metrics.tasks.total} atribuídas</p></div></CardContent></Card>
