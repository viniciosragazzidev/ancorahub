...
import { fetchDistributionLog } from "@/features/lead-distribution/broker-summary-service";
import { encodeDistributionLogPdf } from "@/features/lead-distribution/distribution-log-pdf";
...
export async function GET(request: Request) {
  try {
    ...
    const [rows, branchRow, tenantRow] = await Promise.all([
      fetchDistributionLog(context.tenantId, { startDate: parsed.start, endDate: parsed.end, branchId }),
      branchId ? db.select({ name: schema.branches.name }).from(schema.branches).where(eq(schema.branches.id, branchId)).limit(1) : Promise.resolve([]),
    ...
    const branchLabel = branchId ? (branchRow[0]?.name ?? "Unidade selecionada") : "Todas as unidades";
    const tenant = tenantRow[0];
    const body = await encodeDistributionLogPdf({
      rows,
    ...
    await db.insert(schema.auditLogs).values({ id: randomUUID(), userId: context.userId, entidade: "report", entidadeId: "distribution-summary", acao: `report.generated:distribution-summary:${rows.length}:pdf` });
