...
export function CreateTenantAccessForm({
  action,
  branches,
  tenantId,
}: {
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Input id="memberPassword" name="password" minLength={8} type="password" required />
      </div>
        ...
        <div className="grid gap-2">
          {roles.map((item) => (
            <button
              ...
              className="rounded-lg border border-border bg-muted/30 p-3 text-left transition-colors hover:bg-muted aria-pressed:border-primary aria-pressed:bg-primary/10"
              key={item.value}
