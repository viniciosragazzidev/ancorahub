...
function request(path: string, method = "GET", body?: unknown) {
  return new Promise<ApiResult>((resolve) => chrome.runtime.sendMessage({ type: "API_REQUEST", payload: { path, method, body } }, resolve));
}
...
function showDisconnected(message?: string) {
  ...
  document.querySelector("#connect")!.addEventListener("click", async () => {
    ...
    try {
      const response = await fetch("https://corretop.vercel.app/api/extension/auth/exchange", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ code, deviceId: crypto.randomUUID(), extensionVersion: "0.1.1" }) });
